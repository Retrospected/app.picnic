    ...
    var req = https.request(options, function (res) {
      ...
      res.on('end', function () {
        try {
          ...
          var contexts = JSON.parse(body)["body"]["child"]["analytics"]["contexts"]
          var searchContext = contexts.find(c => c["data"] && c["data"]["product_ids"])
          var productIds = searchContext && searchContext["data"]["product_ids"]
          var productId;
          if (Array.isArray(productIds)) {
            productId = productIds[0];
          } else if (productIds && productIds.type === "EXPRESSION") {
            var arrayMatch = productIds.expression.match(/return (\["[^"]*"(?:,"[^"]*")*\])/)
            productId = arrayMatch ? JSON.parse(arrayMatch[1])[0] : undefined;
          }
