...
class utils extends BaseClass {
  ...
  getOrderStatus() {
    return new Promise((resolve, reject) => {
      ...
      var req = https.request(options, (res) => {
        ...
        res.on('end', () => {
          if (res.statusCode == 401) {
            // "Error: unauthorized" is what the poll compares against and what
            // this Error still reads as, the properties are what a crash
            // report needs to tell it apart from any other failure
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) {
            // a refusal used to be read as order info and fail later on as a
            // parse error, which said nothing about Picnic having refused
      ...
      req.on('timeout', () => {
        // a socket timeout does not end the request by itself, so without this
        // the poll waits on an answer that is never coming
