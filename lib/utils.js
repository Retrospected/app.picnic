  ...
  async addItemToCart(productId, callback) {
    ...
    var req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        if (typeof parsedBody["items"] !== 'undefined' && parsedBody["items"]) {
          parsedBody["items"]
        ...
        else if (parsedBody["error"]["code"] === "UNPROCESSABLE_ENTITY") {
          const matchingItem = parsedBody["items"].find((item) => {
            return item["items"] && item["items"][0] && item["items"][0]['id'] == productId;
          });

          if (matchingItem) {
            callback(matchingItem["items"][0]["name"]);
          } else {
            callback(new Error('Unexpected response from server'));
          }
        }
        else if (parsedBody["error"] && parsedBody["error"]["code"] === "UNPROCESSABLE_ENTITY") {
