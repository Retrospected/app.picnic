...
// The bar fills between the van leaving and the start of its window. A van
// first seen on the road by an app that did not write that moment down (a
// restart mid-route) leaves nothing to fill from, so the last hour before the
 ...
 * @returns {boolean}
 */
function orderCheckDue(state, checkedAt, now, cartEmptied) {
  ...
  return at === null || now - at >= ORDER_MAX_AGE;
}
