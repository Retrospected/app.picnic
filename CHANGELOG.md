...
- **Delivery widget**: Shows when groceries arrive with live countdown timers. Once an order is placed, the widget counts down to the delivery slot. When Picnic announces the exact moment, a bar fills toward delivery time. Shows whether groceries are arriving, running late, or already delivered.
- **Smart syncing**: The widget listens for state changes as they happen, so the countdown is live between polls. It uses Homey's clock instead of depending on the server.
- **Order deadline countdown**: Shows how much time is left to add to the current order, and how much the pending cart is worth. The deadline comes from the delivery window, so it's always accurate, even if Picnic doesn't mention one.
- **Cart awareness**: Between deliveries, the widget shows the cart and product count. If it's below the minimum order, it says how much more is needed. Tap to open Picnic (can be disabled in settings).
- **Efficient fetching**: Cart updates only come when a dashboard is actually showing the widget, and no more than once every five minutes.
- **Correct totals**: Updated the app's API version string so Picnic calculates cart totals correctly, including bundle deals and family discounts.
- **Debugging**: `make run` logs Picnic's responses with personal data blanked out, so you can see what the API returns.
- **Fast order detection**: Orders placed in the Picnic app now reach the widget within minutes instead of hours. While a dashboard is showing, the widget checks for new orders frequently and notices immediately when the cart empties.
- **No false deliveries**: The widget no longer shows phantom deliveries from empty API responses. Only deliveries with a confirmed time window appear on screen.
